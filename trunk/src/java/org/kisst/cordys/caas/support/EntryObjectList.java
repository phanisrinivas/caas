...
public class EntryObjectList<T extends LdapObject> extends CordysObjectList<T>
{
    ...
    {
        return parent.getVarName() + "." + type;
    }

    @Override
    public String getKey()
    {
        return parent.getKey() + ":Entries:" + group;
    }
    ...
    @SuppressWarnings("unchecked")
    public List<String> getDangling()
    {
        fetchList();
        return (List<String>) dangling.clone();
    }

    public void add(LdapObject obj)
    {
        add(obj.getDn());
    }

    public void remove(LdapObject obj)
    {
        remove(obj.getDn());
    }
    ...
    /**
     * update the existing entry with the new ldap objects This will clear the existing child elements and add the new entries to
     * the parent.
     * 
     * @param newEntries
     */
