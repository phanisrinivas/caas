...
/**
 * This class works like a list, without being a real java.util.List This hack is necessary, because in groovy the propertyMissing
 * webService is never used on objects that inherit from a List.
 */
public abstract class CordysObjectList<T extends CordysObject> extends CordysObject implements Iterable<T>
{
    ...
    protected boolean isListAvailable()
    {
    ...
    /**
     * This method will return the object based on the name.
     * <p>
     * Note that if the list is not yet retrieved this method will return null.
     * </p>
     * 
     * @param name
     * @return
    ...
    public boolean contains(LdapObject obj)
    {
        fetchList();
    ...
    public CordysObjectList<T> like(final String filter)
    {
        return new CordysObjectList<T>(system) {
            ...
            {
                return CordysObjectList.this.getKey() + ":like(" + filter + ")";
            }

            @Override
            public Organization getOrganization()
            {
                return CordysObjectList.this.getOrganization();
            }
        };
    }
    ...
    public CordysObjectList<T> sort()
    {
        return new CordysObjectList<T>(system) {
            ...
            public String getKey()
            {
                return CordysObjectList.this.getKey() + ":sorted";
            }
