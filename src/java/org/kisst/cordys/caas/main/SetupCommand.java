...
public class SetupCommand extends CompositeCommand
{
    ...
    public void runold(String args[])
    {
        ...
        Environment.info("Downloading necessary jar files to " + libdir);
        File d = new File(libdir);
        if (!d.exists())
        {
            Environment.info("Making new directory " + libdir);
            d.mkdir();
        }
    }
    ...
    public void downloadGroovy()
    {
        ...
        download(libdir + "groovy-all-1.6.5.jar",
                "http://repo1.maven.org/maven2/org/codehaus/groovy/groovy-all/1.6.5/groovy-all-1.6.5.jar");
    ...
    private static void download(String filename, String url)
    {
        ...
        if (f.exists())
        {
            Environment.info("File " + filename + " allready exists, will not download this");
            return;
        }
        Environment.info("Downloading " + filename + " from " + url);
        InputStream in = null;
