 ...
 * several web services to determine which definitions are available. Also there is a difference between a package definition
 * and a loaded package. This list contains the definitions, not the actually loaded packages.
 */
final class PackageDefinitionList extends CordysObjectList<PackageDefinition>
{
    ...
    {
      //First we need to retrieve the ISV packages that are available on the system. For that we need to call the http://server/cordys/com.eibus.web.application.ListISVPackages.wcp?isvpackage
        ...
        //Execute the request. For this URL we cannot add the SAML token, so in case of the SAML client we need to call a different method.
        String tmp = null;
        ...
        //Now we have the list of all the ISV packages that are loaded on the system. Now we need to get the details for each package.
        ...
        List<XmlNode> caps = response.xpath("./*[local-name()='tuple']/*[local-name()='old']/*[local-name()='ApplicationPackage']");
