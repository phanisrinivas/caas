...
public class ObjectiveBase implements Objective
{
    ...
    private class Entry
    {
        ...
        {
            return "isvp.\"" + isvpName + "\"." + propName + "." + name;
        }
        ...
        public LdapObject findEntry(Organization org)
        {
            ...
            {
                Package pkg = system.packages.getByName(isvpName);
                if (pkg == null)
                {
                    env.error("target " + target.getVarName(org) + " refers to UNKNOWN isvp \"" + isvpName + "\" in entry "
                            + this);
                    return null;
                }
                if (pkg != null)
    ...
    public ObjectiveBase(String propName, Target target, XmlNode node)
    {
        ...
        {
            // It could be that the child nodes are not all of the given prop name. If so they should be ignored
            if (child.getName().equals(propName))
            {
