...
public class Template
{
    ...
    /*
     * private Role[] getRoles(Organization org, XmlNode node) { ArrayList<Role> result=new ArrayList<Role>(); for (XmlNode
     * child:node.getChildren()) { if (child.getName().equals("role")) { Role r=null; String
     * isvpName=child.getAttribute("package"); String roleName=child.getAttribute("name"); String dnRole=null; if (isvpName==null)
     * { r=org.roles.getByName(roleName); dnRole="cn="+roleName+",cn=organizational roles,"+org.getDn(); } else { Isvp
     * isvp=org.getSystem().isvp.getByName(isvpName); if (isvp!=null) r=isvp.roles.getByName(roleName); else
     * dnRole="cn="+roleName+",cn="+isvpName+","+org.getSystem().getDn(); } if (r!=null) result.add(r); else
     * env.error("  skipping unknown role "+roleName); } } return result.toArray(new Role[result.size()]); }
     */
